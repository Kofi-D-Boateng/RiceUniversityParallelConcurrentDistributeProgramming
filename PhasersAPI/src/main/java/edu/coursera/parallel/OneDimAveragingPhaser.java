        ...
        for(int k = 0; k < tasks;k++){
            ...
            threads[k] = new Thread(()->{
               ...
               for(int iter = 0; iter < iterations; iter++){
                   ...
                   threadMyNew[left] = (threadMyVal[left -1] + threadMyVal[left+1]) / 2.0;

                   int right = (i+1) * (n/tasks);
                   threadMyNew[right] = (threadMyVal[right -1] + threadMyVal[right+1]) / 2.0;

                   int currentPhase = phaser.arrive();

                   for(int j = left+1; j<=right-1;j++){
                       threadMyNew[j] = (threadMyVal[j -1] + threadMyVal[j+1]) / 2.0;
                   }

                   phaser.awaitAdvance(currentPhase);

                   double[] temp = threadMyNew;
